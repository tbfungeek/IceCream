...
final class PendingRelationshipsWorker<Element: Object> {
    ...
    func addToPendingList(elementPrimaryKeyValue: AnyHashable, propertyName: String, owner: Object) {
        SyncEnginLogHandler.log(tag: .FetchTags, msg:"[PendingRelationshipsWorker] addToPendingList elementPrimaryKeyValue = \(elementPrimaryKeyValue) propertyName = \(propertyName) owner = \(owner)")
        pendingListElementPrimaryKeyValue[elementPrimaryKeyValue] = (propertyName, owner)
    }
    
    func resolvePendingListElements() {
        
        SyncEnginLogHandler.log(tag: .FetchTags, msg:"[PendingRelationshipsWorker] resolvePendingListElements")
        ...
        BackgroundWorker.shared.start {
            for (primaryKeyValue, (propName, owner)) in self.pendingListElementPrimaryKeyValue {
                ...
                if let existListElementObject = realm.object(ofType: Element.self, forPrimaryKey: primaryKeyValue) {
                    SyncEnginLogHandler.log(tag: .FetchTags, msg:"[PendingRelationshipsWorker] resolvePendingListElements existListElementObject \(existListElementObject) Element = \(Element.self) primaryKeyValue = \(primaryKeyValue)")
                    try! realm.write {
                        list.append(existListElementObject)
                    }
                    self.pendingListElementPrimaryKeyValue[primaryKeyValue] = nil
                } else {
                    SyncEnginLogHandler.log(tag: .FetchTags, msg:"[PendingRelationshipsWorker] resolvePendingListElements primaryKeyValue = \(primaryKeyValue) Cannot find existing resolving record in Realm")
